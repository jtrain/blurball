...
class Tennis:
    def __init__(
        ...
        if self._load_train:
            log.info(
                f"train           | {len(self._train_all):7d} | {self._train_num_frames:7d} | {self._train_num_frames_with_gt:13d} | {self._train_num_rallies:7d} | {self._train_num_matches:7d} | {self._train_disp_mean:2.1f}+/-{self._train_disp_std:2.1f}"
            )
        if self._load_train_clip:
            num_items_all = 0
            num_frames_all = 0
            ...
            for key, clip in self._train_clips.items():
                ...
                clip_name = f"{key[0]}_{key[1]}"
                disps = np.array(self._train_clip_disps[key])
                log.info(
                    f"{clip_name} | {num_items:7d} | {num_frames:7d} | {num_frames_with_gt:13d} |         |         | {np.mean(disps):2.1f}+/-{np.std(disps):2.1f}"
                )
                ...
                num_clips_all += 1
            log.info(
                f"all         | {num_items_all:7d} | {num_frames_all:7d} | {num_frames_with_gt_all:13d} | {num_clips_all:7d} |         | {np.mean(disps_all):2.1f}+/-{np.std(disps_all):2.1f}"
            )
        if self._load_test:
            log.info(
                f"test            | {len(self._test_all):7d} | {self._test_num_frames:7d} | {self._test_num_frames_with_gt:13d} | {self._test_num_rallies:7d} | {self._test_num_matches:7d} | {self._test_disp_mean:2.1f}+/-{self._test_disp_std:2.1f}"
            )
        if self._load_test_clip:
            num_items_all = 0
            num_frames_all = 0
    ...
    def _gen_seq_list(
        ...
        if refine_npz_path is not None:
            log.info(f"refine gt ball positions with {refine_npz_path}")
        ...
        for match in matches:
            ...
            for clip_name in clip_names:
                ...
                ball_xyvs = load_csv(
                    clip_csv_path, self._visible_flags, frame_dir=clip_frame_dir
                )
                frame_names = []
